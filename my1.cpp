...
int main(int argc, char **argv) {
    ...
    int dop_size = size + 2;
    vector<double> dop_stiffnessMatrix(dop_size * dop_size, 0);
    vector<double> dop_loadVector(dop_size, 0);
    vector<double> dop_displacements(dop_size, 0);
    // Копируем значения из stiffnessMatrix в dop_stiffnessMatrix
    for (int i = 0; i < size; ++i) {
        for (int j = 0; j < size; ++j) {
            dop_stiffnessMatrix[i * dop_size + j] = stiffnessMatrix[i * size + j];
        }
    }
    for (int i = 0; i < size; ++i) {
        dop_loadVector[i] = loadVector[i];
    }
    dop_stiffnessMatrix[dop_size - 2] = -a;
    dop_stiffnessMatrix[dop_size * (size - 1) + dop_size - 1] = a;
    dop_stiffnessMatrix[dop_size * size] = 1;
    dop_stiffnessMatrix[dop_size * dop_size - 1] = 1;
    dop_loadVector[dop_size - 2] = 0;
    dop_loadVector[dop_size - 1] = 4;
    
    solveSLAU(dop_displacements, dop_stiffnessMatrix, dop_loadVector, dop_size);
    for (int i = 0; i < dop_size; i++) {
        cout << i << " " << dop_displacements[i] << " ";
    }
    // for (int i = 0; i < dop_size * dop_size; i++) {
    //     cout << i << " " << dop_stiffnessMatrix[i] << endl;
    // }
    // for (int i = 0; i < dop_size; i++) {
    //     cout << i << " " << dop_loadVector[i] << endl;
    // }


    // Применяем граничные условия
    // restrictType1(stiffnessMatrix, loadVector, a, size, 0, lower.val);
    restrictType2(stiffnessMatrix, loadVector, a, size, 0, dop_displacements[dop_size - 2]);
